...
const createUser = async (req, res) => {
    ...
    const userExist = await userModel_1.default.findOne({ email: value.email });
    if (userExist) {
        return res.status(400).json({
            success: false,
    ...
    const user = new userModel_1.default(Object.assign(Object.assign({}, value), { password: hashedPassword }));
    const saveduser = await user.save();
    const token = jsonwebtoken_1.default.sign({ userId: saveduser.id }, process.env.JWT_SECRET);
    const findUser = await userModel_1.default.findById(saveduser.id);
...
const login = async (req, res) => {
    try {
        ...
        const user = await userModel_1.default.findOne({ email: value.email }).select('+password');
        if (!user) {
            return res.status(400).json({
                success: false,
...
const updateUser = async (req, res) => {
    try {
        ...
        await userModel_1.default.findByIdAndUpdate(req.userId, Object.assign({}, value));
        return res.json({
            success: true,
